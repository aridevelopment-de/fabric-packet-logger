...
public class LoginSuccessS2CPacketHandler implements BasePacketHandler<LoginSuccessS2CPacket> {
    ...
    @Override
    public String url() {
        return "htthttps://wiki.vg/index.php?title=Protocol&oldid=18067#Login_Success";
    }
    ...
    public JsonObject description() {
        ...
        jsonObject.addProperty("wikiVgNotes", "The (notchian) server might take a bit to fully transition to the Play state, so it's recommended to wait for the Login (play) packet from the server.\bThe notchian client doesn't send any packets until the Login (play) packet (htthttps://wiki.vg/index.php?title=Protocol&oldid=18067#Login_.28play.29).");
